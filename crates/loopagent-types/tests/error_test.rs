use loopagent_types::error::{
    ConfigError, HookError, LoopAgentError, ProviderError, StorageError, ToolError,
};
...
#[test]
fn test_loopagent_error_is_rate_limited_true() {
    let err = LoopAgentError::Provider(ProviderError::RateLimited {
        retry_after_ms: 1000,
    });
    assert!(err.is_rate_limited());
}
...
#[test]
fn test_loopagent_error_is_rate_limited_false_for_non_provider() {
    let err = LoopAgentError::Other("something".into());
    assert!(!err.is_rate_limited());
}

#[test]
fn test_loopagent_error_is_rate_limited_false_for_non_rate_limit_provider() {
    let err = LoopAgentError::Provider(ProviderError::Http("err".into()));
    assert!(!err.is_rate_limited());
}

#[test]
fn test_loopagent_error_retry_after_ms_some() {
    let err = LoopAgentError::Provider(ProviderError::RateLimited {
        retry_after_ms: 2000,
    });
...
#[test]
fn test_loopagent_error_retry_after_ms_none() {
    let err = LoopAgentError::Permission("denied".into());
    assert_eq!(err.retry_after_ms(), None);
}
...
#[test]
fn test_loopagent_error_display_provider() {
    let err = LoopAgentError::Provider(ProviderError::StreamEnded);
    assert_eq!(
        format!("{err}"),
...
#[test]
fn test_loopagent_error_display_tool() {
    let err = LoopAgentError::Tool(ToolError::NotFound("foo".into()));
    assert_eq!(format!("{err}"), "Tool error: Tool not found: foo");
}

#[test]
fn test_loopagent_error_display_config() {
    let err = LoopAgentError::Config(ConfigError::MissingField("model".into()));
    assert_eq!(
        format!("{err}"),
...
#[test]
fn test_loopagent_error_display_storage() {
    let err = LoopAgentError::Storage(StorageError::SessionNotFound("abc".into()));
    assert_eq!(format!("{err}"), "Storage error: Session not found: abc");
}

#[test]
fn test_loopagent_error_display_permission() {
    let err = LoopAgentError::Permission("not allowed".into());
    assert_eq!(format!("{err}"), "Permission denied: not allowed");
}

#[test]
fn test_loopagent_error_display_hook() {
    let err = LoopAgentError::Hook(HookError::Rejected("blocked".into()));
