...
fn test_text_content_delta() {
    let (chunks, _) =
        parse_event(r#"{"choices":[{"delta":{"content":"Hello world"},"finish_reason":null}]}"#);
...
fn test_tool_calls_accumulation() {
    ...
    let _ = parse_with_state(
        r#"{"choices":[{"delta":{"tool_calls":[{"index":0,"function":{"arguments":"{\"path\":"}}]},"finish_reason":null}]}"#,
        &mut state,
    );
    let _ = parse_with_state(
        r#"{"choices":[{"delta":{"tool_calls":[{"index":0,"function":{"arguments":"\"foo.rs\"}"}}]},"finish_reason":null}]}"#,
        &mut state,
    );
...
fn test_finish_reason_stop_with_accumulated_tools() {
    ...
    state
        .calls
        .push(("call_x".to_string(), "tool_x".to_string(), "{}".to_string()));
...
fn test_usage_stats() {
    let (chunks, _) =
        parse_event(r#"{"usage":{"prompt_tokens":150,"completion_tokens":42},"choices":[]}"#);
    assert_eq!(chunks.len(), 1);
    match &chunks[0] {
...
fn test_large_tool_call_index_skipped() {
    let (chunks, state) = parse_event(
        r#"{"choices":[{"delta":{"tool_calls":[{"index":200,"id":"call_big","function":{"name":"evil","arguments":"{}"}}]},"finish_reason":null}]}"#,
    );
    assert!(chunks.is_empty());
    assert!(state.calls.is_empty());
}
...
fn test_usage_null_is_ignored() {
    let (chunks, _) = parse_event(
        r#"{"usage":null,"choices":[{"delta":{"content":"text"},"finish_reason":null}]}"#,
    );
    assert_eq!(chunks.len(), 1);
...
fn test_finish_tool_calls_skips_empty_id_name() {
    ...
    state
        .calls
        .push(("".to_string(), "".to_string(), "{}".to_string()));
    ...
    assert!(
        chunks.is_empty(),
        "empty id+name tool calls should be skipped"
    );
}
