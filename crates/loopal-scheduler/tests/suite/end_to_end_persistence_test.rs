...
//! them to a real `FileScopedCronStore`; a second scheduler pointing at
//! the same root + session id calls `load_persisted` and rehydrates them.

use std::sync::Arc;

use chrono::{TimeZone, Utc};
use loopal_scheduler::{CronScheduler, FileScopedCronStore, ManualClock, SessionScopedCronStorage};
...
async fn durable_tasks_survive_scheduler_restart() {
    ...
    // --- Writer scheduler ----------------------------------------
    let store_a: Arc<dyn SessionScopedCronStorage> =
        Arc::new(FileScopedCronStore::new(dir.path().to_path_buf()));
    let clock_a = Arc::new(ManualClock::new(
        Utc.with_ymd_and_hms(2026, 4, 10, 10, 0, 0).unwrap(),
    ));
    let writer = CronScheduler::with_session_storage_and_clock(store_a, clock_a);
    writer.switch_session(SESSION).await.expect("bind");
    ...
    // --- Reader scheduler ----------------------------------------
    let store_b: Arc<dyn SessionScopedCronStorage> =
        Arc::new(FileScopedCronStore::new(dir.path().to_path_buf()));
...
async fn load_persisted_cleans_up_missed_one_shot() {
    ...
    let sched = CronScheduler::with_session_storage_and_clock(store_a, clock_a);
    sched.switch_session(SESSION).await.expect("bind");
    sched
    ...
    let path = dir.path().join(SESSION).join("cron.json");
