//! JSON-RPC 2.0 transport for ACP.
//!
//! Types and parsing are re-exported from `loopal-ipc`. This module retains
//! `JsonRpcTransport` — the ACP-specific writer with pending-request routing.
//! Migration to `loopal_ipc::Connection` will happen in Phase 2.
...
// ── ACP Transport (writer + pending requests) ────────────────────────

/// Newline-delimited JSON-RPC transport for ACP (stdout-based).
///
/// Will be replaced by `loopal_ipc::Connection` in Phase 2 of the
