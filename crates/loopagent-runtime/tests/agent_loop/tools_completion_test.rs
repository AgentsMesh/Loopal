...
use loopagent_types::message::ContentBlock;
...
/// Fake tool that returns "Completion acknowledged: {input.result}".
...
impl Tool for FakeCompletionTool {
    ...
    async fn execute(
        ...
        Ok(ToolResult::success(format!("Completion acknowledged: {result}")))
    }
}
