...
async fn op_subscribe(shared: &Arc<AgentShared>, channel: Option<&str>) -> Result<ToolResult, LoopAgentError> {
...
async fn op_unsubscribe(shared: &Arc<AgentShared>, channel: Option<&str>) -> Result<ToolResult, LoopAgentError> {
    let ch = require_channel(channel)?;
...
async fn op_publish(
    ...
    let message = input.get("message").and_then(|v| v.as_str())
        .ok_or_else(|| LoopAgentError::Tool(
            loopagent_types::error::ToolError::InvalidInput("missing 'message'".into()),
        ))?;
...
async fn op_list(shared: &Arc<AgentShared>) -> Result<ToolResult, LoopAgentError> {
...
fn require_channel(ch: Option<&str>) -> Result<&str, LoopAgentError> {
    ch.ok_or_else(|| LoopAgentError::Tool(
        loopagent_types::error::ToolError::InvalidInput("missing 'channel'".into()),
    ))
}
