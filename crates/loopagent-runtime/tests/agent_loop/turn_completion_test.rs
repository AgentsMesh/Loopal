...
use loopagent_types::error::{LoopAgentError, TerminateReason};
use loopagent_types::permission::{PermissionLevel, PermissionMode};
use loopagent_types::provider::{ChatParams, ChatStream, Provider, StopReason, StreamChunk};
use loopagent_types::tool::{Tool, ToolContext, ToolResult};
use tokio::sync::mpsc;

// --- Multi-call mock provider ---

struct MultiMockStream(VecDeque<Result<StreamChunk, LoopAgentError>>);
...
struct MultiCallProvider {
    calls: std::sync::Mutex<VecDeque<Vec<Result<StreamChunk, LoopAgentError>>>>,
}
impl MultiCallProvider {
    fn new(calls: Vec<Vec<Result<StreamChunk, LoopAgentError>>>) -> Self {
...
impl Tool for FakeCompletionTool {
    ...
    async fn execute(&self, input: serde_json::Value, _ctx: &ToolContext,
        ...
        Ok(ToolResult::success(format!("Completion acknowledged: {r}")))
    }
}
...
) -> (AgentLoopRunner, mpsc::Receiver<loopagent_types::event::AgentEvent>) {
...
async fn test_stream_error_after_tool_preserves_last_text() {
    ...
    let calls = vec![
        ...
        vec![
            Err(LoopAgentError::Provider(
                loopagent_types::error::ProviderError::StreamEnded,
            )),
        ],
    ];
    let (mut runner, mut event_rx) = make_multi_runner(calls, false);
