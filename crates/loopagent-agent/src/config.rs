...
/// Configuration for a sub-agent type, loaded from `.loopagent/agents/*.md`.
#[derive(Debug, Clone)]
...
pub fn load_agent_configs(cwd: &Path) -> HashMap<String, AgentConfig> {
    ...
    // Global agents dir: ~/.loopagent/agents/
    if let Ok(global_dir) = loopagent_config::global_agents_dir() {
        load_configs_from_dir(&global_dir, &mut map);
    }

    // Project agents dir: <cwd>/.loopagent/agents/
    let project_dir = loopagent_config::project_agents_dir(cwd);
