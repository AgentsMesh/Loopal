...
fn render_handles_missing_variable_gracefully() {
    ...
    let registry = loopal_prompt::FragmentRegistry::new(vec![
...
fn render_jinja_condition() {
    ...
    assert_eq!(registry.render(&registry.fragments()[0], &ctx_with), "HAS_BASH");

    let ctx_without = PromptContext::default();
    assert_eq!(registry.render(&registry.fragments()[0], &ctx_without), "NO_BASH");
}
