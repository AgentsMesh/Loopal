...
struct TextOnlyProvider {
    chunks: std::sync::Mutex<Option<Vec<Result<StreamChunk, LoopAgentError>>>>,
}
...
struct MockStream {
    chunks: VecDeque<Result<StreamChunk, LoopAgentError>>,
}
...
async fn test_subagent_drains_pending_before_exit() {
    ...
    let params = AgentLoopParams {
        kernel,
        session,
        messages: vec![loopagent_types::message::Message::user("run task")],
        model: "claude-sonnet-4-20250514".into(),
