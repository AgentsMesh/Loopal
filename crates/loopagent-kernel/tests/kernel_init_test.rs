...
fn test_resolve_api_key_direct() {
    ...
    let result =
        loopagent_kernel::resolve_api_key(&Some("my-direct-key".to_string()), &None);
...
fn test_resolve_api_key_env_var() {
    ...
    let result =
        loopagent_kernel::resolve_api_key(&None, &Some(env_var_name.to_string()));
...
fn test_resolve_api_key_direct_over_env() {
    ...
    let result = loopagent_kernel::resolve_api_key(
...
fn test_resolve_api_key_empty() {
    ...
    let result = loopagent_kernel::resolve_api_key(&Some(String::new()), &None);
    assert_eq!(result, None);

    // Both None
    let result = loopagent_kernel::resolve_api_key(&None, &None);
    assert_eq!(result, None);
