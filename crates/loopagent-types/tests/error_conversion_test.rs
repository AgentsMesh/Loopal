//! Tests for LoopAgentError From conversions and sub-error Display impls.

use loopagent_types::error::{
    ConfigError, HookError, LoopAgentError, McpError, ProviderError, StorageError, ToolError,
};

#[test]
fn test_loopagent_error_display_mcp() {
    let err = LoopAgentError::Mcp(McpError::ServerNotFound("test-server".into()));
    assert_eq!(
        format!("{err}"),
...
#[test]
fn test_loopagent_error_display_other() {
    let err = LoopAgentError::Other("unexpected".into());
...
#[test]
fn test_loopagent_error_from_provider_error() {
    let provider_err = ProviderError::StreamEnded;
    let err: LoopAgentError = provider_err.into();
    assert!(matches!(err, LoopAgentError::Provider(_)));
}

#[test]
fn test_loopagent_error_from_tool_error() {
    let tool_err = ToolError::Timeout(5000);
    let err: LoopAgentError = tool_err.into();
    assert!(matches!(err, LoopAgentError::Tool(_)));
}

#[test]
fn test_loopagent_error_from_config_error() {
    let config_err = ConfigError::Parse("bad toml".into());
    let err: LoopAgentError = config_err.into();
    assert!(matches!(err, LoopAgentError::Config(_)));
}

#[test]
fn test_loopagent_error_from_hook_error() {
    let hook_err = HookError::Timeout("slow hook".into());
    let err: LoopAgentError = hook_err.into();
    assert!(matches!(err, LoopAgentError::Hook(_)));
}

#[test]
fn test_loopagent_error_from_mcp_error() {
    let mcp_err = McpError::ConnectionFailed("refused".into());
    let err: LoopAgentError = mcp_err.into();
    assert!(matches!(err, LoopAgentError::Mcp(_)));
}
