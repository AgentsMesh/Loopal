use loopagent_config::load_settings;
use tempfile::TempDir;

#[test]
fn test_load_settings_all_env_var_scenarios() {
    // Combined test to avoid env var race conditions between parallel tests.
    // All subtests that touch LOOPAGENT_* env vars are serialized here.
    unsafe {
        std::env::remove_var("LOOPAGENT_MODEL");
        std::env::remove_var("LOOPAGENT_MAX_TURNS");
        std::env::remove_var("LOOPAGENT_PERMISSION_MODE");
    }
    ...
    {
        let tmp = TempDir::new().unwrap();
        let config_dir = tmp.path().join(".loopagent");
    ...
    {
        unsafe {
            std::env::set_var("LOOPAGENT_MODEL", "test-model");
            std::env::set_var("LOOPAGENT_MAX_TURNS", "10");
        }
    ...
    // --- Scenario 6: Non-numeric LOOPAGENT_MAX_TURNS is ignored ---
    {
        unsafe {
            std::env::set_var("LOOPAGENT_MAX_TURNS", "not_a_number");
        }
