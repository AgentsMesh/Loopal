...
use loopagent_types::error::{ConfigError, LoopAgentError};
...
fn load_json_file(path: &Path) -> Result<serde_json::Value, LoopAgentError> {
...
fn apply_env_overrides(value: &mut serde_json::Value) {
    ...
    if let Ok(model) = std::env::var("LOOPAGENT_MODEL") {
        value["model"] = serde_json::Value::String(model);
    }

    if let Ok(max_turns) = std::env::var("LOOPAGENT_MAX_TURNS")
        && let Ok(n) = max_turns.parse::<u32>() {
            value["max_turns"] = serde_json::json!(n);
        }

    if let Ok(mode) = std::env::var("LOOPAGENT_PERMISSION_MODE") {
...
pub fn load_settings(cwd: &Path) -> Result<Settings, LoopAgentError> {
...
/// Load and concatenate instruction files (LOOPAGENT.md).
/// Global instructions come first, then project instructions, separated by newlines.
pub fn load_instructions(cwd: &Path) -> Result<String, LoopAgentError> {
...
mod tests {
    ...
    fn test_load_json_file_not_found_returns_null() {
        ...
        let path = std::path::Path::new("/tmp/loopagent_test_nonexistent_file_xyz_12345.json");
