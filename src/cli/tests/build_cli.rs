...
#[test]
fn desktop_serve_subcommand_is_registered_without_changing_vault_commands() {
    let cmd = build_cli();
    let desktop = cmd
        .get_subcommands()
        .find(|c| c.get_name() == "desktop")
        .expect("desktop subcommand must be attached");
    assert!(
        desktop.get_subcommands().any(|c| c.get_name() == "serve"),
        "desktop must expose the stable serve command"
    );
}
