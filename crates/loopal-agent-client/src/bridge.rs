...
//!
//! Test-only fixture in the Hub-first architecture: production agents use
//! `HubFrontend` and never enter this code. Bridge is retained as IPC
//! protocol-layer test scaffolding for `loopal-ipc::e2e_ipc_*` tests.
...
pub fn start_bridge(
    ...
    tokio::spawn(async move {
        bridge_incoming(incoming_rx, conn_in, agent_event_tx).await;
    });
    ...
    loop {
        ...
        match incoming {
            ...
            Incoming::Request { id, method, .. } => {
                debug!(id, %method, "bridge: unknown incoming request");
                let _ = connection
                    .respond_error(
                        id,
                        loopal_ipc::jsonrpc::METHOD_NOT_FOUND,
                        &format!("bridge does not handle: {method}"),
                    )
                    .await;
            }
        }
