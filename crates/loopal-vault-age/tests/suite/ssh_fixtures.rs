// SSH key fixtures from the public age crate test suite (age-0.10.1 src/ssh/identity.rs
// and src/ssh/recipient.rs). These are PUBLIC test keys, not anyone's real credentials.
// Used here to exercise our parse + permission paths without requiring ssh-keygen at test time.

pub const ED25519_UNENCRYPTED: &str = "-----BEGIN OPENSSH PRIVATE KEY-----

-----END OPENSSH PRIVATE KEY-----
";

pub const ED25519_AES256_CBC: &str = "-----BEGIN OPENSSH PRIVATE KEY-----

-----END OPENSSH PRIVATE KEY-----
";

pub const RSA_PRIVATE: &str = "-----BEGIN RSA PRIVATE KEY-----

-----END RSA PRIVATE KEY-----
";
