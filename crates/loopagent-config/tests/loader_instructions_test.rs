use loopagent_config::load_instructions;
use tempfile::TempDir;

#[test]
fn test_load_instructions() {
    let tmp = TempDir::new().unwrap();
    std::fs::write(tmp.path().join("LOOPAGENT.md"), "# Project Instructions").unwrap();
...
fn test_load_instructions_project_only() {
    let tmp = TempDir::new().unwrap();
    std::fs::write(tmp.path().join("LOOPAGENT.md"), "# Project\nDo stuff.").unwrap();
