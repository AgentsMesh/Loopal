...
#[test]
fn test_get_opus_4_8_model() {
    let info = get_model_info("claude-opus-4-8").unwrap();
    assert_eq!(info.provider, "anthropic");
...
#[test]
fn test_opus_4_8_resolves_to_anthropic() {
    assert_eq!(resolve_provider("claude-opus-4-8"), "anthropic");
}
