...
fn finished_clears_running_agent() {
    ...
    apply_event(&mut state, AgentEvent::named("sub1", AgentEventPayload::Started));
    apply_event(
        &mut state,
        AgentEvent::named("sub1", AgentEventPayload::ThinkingStream { text: "...".into() }),
    );
    assert_eq!(state.agents["sub1"].observable.status, AgentStatus::Running);

    // Simulate disconnect → synthetic Finished event
    apply_event(&mut state, AgentEvent::named("sub1", AgentEventPayload::Finished));
    assert_eq!(state.agents["sub1"].observable.status, AgentStatus::Finished);
}
