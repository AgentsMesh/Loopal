...
const TOOL_NAME: &str = "request_idle";
...
impl AgentLoopRunner {
    pub(super) async fn handle_request_idle(
        ...
        if let Err(msg) = validate_duration(parsed.max_idle_duration_secs) {
            return self
                .complete_intercepted_tool(idx, id, TOOL_NAME, &msg, true, None)
                .await;
        }
        ...
        self.complete_intercepted_tool(idx, id, TOOL_NAME, &message, false, None)
            .await
    }
}
