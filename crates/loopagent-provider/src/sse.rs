...
pub struct SseStream {
    inner: Pin<Box<dyn Stream<Item = Result<String, LoopAgentError>> + Send>>,
}
    ...
    pub fn from_response(response: Response) -> Self {
        ...
        let mapped = event_stream.filter_map(|result| {
            let out = match result {
                ...
                Err(e) => Some(Err(LoopAgentError::Provider(ProviderError::SseParse(
                    e.to_string(),
                )))),
            };
...
impl Stream for SseStream {
    type Item = Result<String, LoopAgentError>;
