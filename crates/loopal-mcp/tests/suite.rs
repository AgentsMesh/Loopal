#[path = "suite/manager_test.rs"]
mod manager_test;

#[path = "suite/client_test.rs"]
mod client_test;

#[path = "suite/connection_test.rs"]
mod connection_test;

#[path = "suite/types_test.rs"]
mod types_test;

#[path = "suite/callback_test.rs"]
mod callback_test;

#[path = "suite/snapshot_test.rs"]
mod snapshot_test;

#[path = "suite/proxy_client_support.rs"]
mod proxy_client_support;

#[path = "suite/proxy_client_test.rs"]
mod proxy_client_test;

#[path = "suite/proxy_client_security_test.rs"]
mod proxy_client_security_test;

#[path = "suite/local_provider_test.rs"]
mod local_provider_test;

#[path = "suite/ipc_budget_test.rs"]
mod ipc_budget_test;
