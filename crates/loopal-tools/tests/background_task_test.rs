...
use loopal_tools::builtin::background::task_output::TaskOutputTool;
use loopal_tools::builtin::background::task_stop::TaskStopTool;
use loopal_tools::builtin::background::{self, BackgroundTask, TaskStatus};
...
fn test_store_insert_and_retrieve() {
    let store = background::store();
    let task_id = background::generate_task_id();
...
fn test_generate_task_id_is_unique() {
    let id1 = background::generate_task_id();
    let id2 = background::generate_task_id();
    assert_ne!(id1, id2);
