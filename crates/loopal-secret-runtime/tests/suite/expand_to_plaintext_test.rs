...
async fn substitutes_plaintext_on_success() {
    ...
    let out = expand_to_plaintext("key={{secret:api_key}}", c.as_ref()).await;
...
async fn falls_back_to_placeholder_on_not_found() {
    ...
    let out = expand_to_plaintext("k={{secret:missing}}", c.as_ref()).await;
...
async fn falls_back_to_placeholder_on_ipc_error() {
    ...
    let out = expand_to_plaintext("k={{secret:any}}", c.as_ref()).await;
    assert_eq!(
...
async fn no_placeholder_passes_through_unchanged() {
    let c = client(MockBehavior::IpcError);
    let out = expand_to_plaintext("plain text", c.as_ref()).await;
...
async fn secret_string_is_zeroized_after_use() {
    ...
    let out = expand_to_plaintext("v={{secret:x}}", c.as_ref()).await;
    assert!(out.contains("super-secret"));
    // The intermediate SecretString in the resolver HashMap drops at function
    // return; we can't assert zeroization directly without unsafe inspection,
    // but exposing through ExposeSecret demonstrates the API contract.
